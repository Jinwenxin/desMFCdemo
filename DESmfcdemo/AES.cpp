...
void AES::Encrypt_File(string plainFile, unsigned char* key, string cipherFile)
{
	...
	while(!plain.eof())
	{
		...
		if(count == 16)
		{
			...
			cipher.write(cBlock, 16);
		}
	}
	//如果最后一个明文块不足128bit，则进行补位操作
	if(count)
	{
		plainBlock = CharptrToUnsignedptr(pBlock, count);
		PKCS7Padding(count, plainBlock);
...
void AES::Decrypt_File(string cipherFile, unsigned char* key, string plainFile)
{
	...
	while(1)
	{
		...
		//cipherBlock = (unsigned char *) cBlock;
		cipherBlock = CharptrToUnsignedptr(cBlock, 16);
		plainBlock = InvCipher(cipherBlock);
		pBlock = UnsignedptrToCharptr(plainBlock, 16);
		if(cipher.peek() == EOF)
			break;
		else
		{
...
void AES::PKCS7Padding(int count, unsigned char block[16])
{
	if(count)
	{
		for(int i = count; i < 16; i++)
		{
			...
			//for(int i = 0; i < 4; i++) printf("%0x ", tmp[i]); cout << endl;
			block[i] = (16 - count) & 0xff;
			//cout << tmp << endl;
			//strcpy(block[i], tmp);
			//block[i] = tmp;
		}
...
char *AES::UnsignedptrToCharptr(unsigned char *unc, int length)
{
	char *c = new char[16];
...
unsigned char *AES::CharptrToUnsignedptr(char *c, int length)
{
	unsigned char *unc = new unsigned char[16];
	for(int i = 0; i < length; i++)
		unc[i] = c[i];
	return unc;
}