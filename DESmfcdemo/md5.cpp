...
void MD5_Read(const char *r,string &md5str)
{
	MD5_CTX md5;
	MD5Init(&md5);                          //初始化用于md5加密的结构

	unsigned char buffer[64]; //存放于加密的信息
	char encrypt[64];			//用于read读取文本
	unsigned char decrypt[16];       //存放每次加密后的结果
	...
	{	
		...
		MD5Final(decrypt,&md5);                            //获得每一次结果

		/*-----------------每一次得到的decrypt中的128位用于更新state[0]~state[3]------------------*/

		md5.state[0]=((((((md5.state[0]+decrypt[0])<<8)+decrypt[1])<<8)+decrypt[2])<<8)+decrypt[3];
		md5.state[1]=((((((md5.state[1]+decrypt[4])<<8)+decrypt[5])<<8)+decrypt[6])<<8)+decrypt[7];
		md5.state[2]=((((((md5.state[2]+decrypt[8])<<8)+decrypt[9])<<8)+decrypt[10])<<8)+decrypt[11];
		md5.state[3]=((((((md5.state[3]+decrypt[12])<<8)+decrypt[13])<<8)+decrypt[14])<<8)+decrypt[15];
